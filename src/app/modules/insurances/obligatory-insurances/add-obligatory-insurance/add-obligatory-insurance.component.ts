...
export class AddObligatoryInsuranceComponent implements OnInit {
    ...
    formGroup = new FormGroup({
        ...
        taxe1: new FormControl(0, [Validators.required, Validators.min(0)/*, Validators.max(100)*/]),
        taxe2: new FormControl(0, [Validators.required, Validators.min(0)/*, Validators.max(100)*/]),
        taxe3: new FormControl(0, [Validators.required, Validators.min(0)/*, Validators.max(100)*/]),
    ...
    cards: Card[] = [
        ...
        },
        {
            ...
            fields: [
                {
                    hasAction: 'yes',
                    action: this.ChangeTaxe.bind(this),
                    form: {
                        type: "number",
                        label: "القسط الصافي",
                        hidden: false,
    ...
    calculateTax() {
        ...
        let fr=this.myForm.formGroup;
        let taxes = TaxesHelper.calculateObligatoryInsuranceTax(fr.get('description')?.value,fr.get('insurance_type')?.value,fr.get('permission_type')?.value,fr.get('car_charge')?.value,fr.get('car_nb_passager')?.value)
        fr.get('initial')?.setValue(taxes.initial)
        fr.get('taxe1')?.setValue(taxes.taxe1)
        fr.get('taxe2')?.setValue(taxes.taxe2)
        fr.get('taxe3')?.setValue(taxes.taxe3)
        fr.get('taxe4')?.setValue(taxes.taxe4)
        fr.get('total')?.setValue(taxes.total)
    }

    ChangeTaxe(){
        let fr=this.myForm.formGroup;
        fr.get('total')?.setValue(Number(fr.get('taxe1')?.value)+Number(fr.get('taxe2')?.value)+Number(fr.get('taxe3')?.value)+Number(fr.get('taxe4')?.value)+Number(fr.get('initial')?.value))
    }
